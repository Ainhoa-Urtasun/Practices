...
txt = [num if (i + 1) % 10 == 0 else '' for i,num in enumerate(mydata['Nominal unit labour cost based on hours worked'])]
for i in numpy.arange(0,len(txt)):
  plt.annotate(txt[i],(ticks[i],mydata['Nominal unit labour cost based on hours worked'][i]),textcoords="offset points",ha='center',color='blue')
txt = [num if (i + 1) % 10 == 0 else '' for i,num in enumerate(mydata['Real labour productivity per hour worked'])]
for i in numpy.arange(0,len(txt)):
